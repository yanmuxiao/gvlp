...
var XjQuery = (function(){
	...
	// actionSheet begin
	// 参数：asClass==>弹出actionSheet按钮的class，config==>对应的执行方法
	var actionMask = function(asClass, config){
		var defaults = {
			asList : ['提交', '保存'],
			asFn1 : function(){
        		console.log('提交');
		...
		var asHTML = 	'<div class="xlg-mask"></div>' +
						'<div class="as-wrap">' +
		...
		oBody.append(asHTML);
		// 禁止点击和滑动冒泡
		$(".xlg-mask").on('touchmove',function(event) { 
			event.stopPropagation();
			event.preventDefault(); 
		}, false); 
        ...
        $('.xlg-mask,.as-cancle').each(function(){
        	util.toucher(this).on('singleTap', function(){
        		hideActionSheet();
        	})
        })
	...
	// popverMenu
	var popverMenu = function(popoverCID,config){
		var defaults = {
		...
		var popoverHTML =	'<div class="xlg-mask" style="background-color: rgba(0,0,0,0);"></div>' +
							'<div class="xlg-popover" style="' + option.popoverStyle + '">' + 
								'<div class="triangle_border_up" style="' + option.triangleStyle + '"></div>' + 
								'<ul class="xlg-popover-ul">' +
								asulliHTML +
								'</ul>' + 
							'</div>';
		oBody.append(popoverHTML);
		var  popoverObj = $('.xlg-popover');
		var  maskObj = $(".xlg-mask");
		// 禁止点击和滑动冒泡
		util.toucher($(".xlg-mask")[0]).on('swipeStart',function() { 
			popoverObj.hide();
			maskObj.hide();
			setTimeout(function(){
				oBody.removeClass('pointer-event-none');	
			},150);
		}, false); 
		$(".xlg-popover").on('touchmove',function(event) {
			event.stopPropagation();
			event.preventDefault(); 
		}, false); 
		 //显示 
		util.toucher($(popoverCID)[0]).on('singleTap', function(){
			popoverObj.show();
			maskObj.show();
			oBody.addClass('pointer-event-none');	
		});
		// 隐藏
		...
		// 事件绑定
		$('.xlg-popover-ul li').each(function(){
			util.toucher(this).on('singleTap', function(){
	...
	var swipeFixedVertical = function(wrapID, contentClass){
		var start_top;
		util.toucher(document.getElementById(wrapID))
		.on('swipeStart',contentClass,function(){
			...
			if($('#xlg-body').scrollTop() == 0 && e.moveY > 0){
				$(this).css({
					'-webkit-transition-duration' : '0ms',
